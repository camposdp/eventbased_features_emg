...
def process_data_windowed(signals, labels, window_size, step, feature_list):
    ...
    for start in range(0, n_samples - window_size + 1, step):
        ...
        for feat_type in feature_list:
            feats = extract_feature(window, feat_type)  # Retorna vetor (12 valores, um por canal)
...
def main():
    ...
    for exercise in exercises:
        ...
        X_all_features, y = process_data_windowed(signals, labels, window_size, step, feature_types)
        ...
        df_history.to_csv(f"ffs_history_{exercise}.csv", index=False)
        ...
        plt.title(f'Forward Feature Selection - {exercise}')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"grafico_ffs_{exercise}.png")
        plt.close()
        ...
        df_folds.to_csv(f"folds_result_{exercise}.csv", index=False)
        ...
        plt.xlabel('Fold')
        plt.ylabel('Acurácia')
        plt.title(f'Validação Cruzada 10-Fold - {exercise}')
        plt.grid(axis='y')
        plt.tight_layout()
        plt.savefig(f"grafico_folds_{exercise}.png")
        plt.close()
        ...
        joblib.dump(final_model, f"modelo_final_{exercise}.pkl")
